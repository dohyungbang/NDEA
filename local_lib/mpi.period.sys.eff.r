mpi.period.sys.eff <- function(dea_data, epsilon, agg.sys.eff, idx.var, time.var, x.vars, z.vars, y.vars){
  ...
  for (dmu_k in dmu_list){
    ...
    fixed_eff_constr_mat <- c(-agg_eff_s*(agg_sub_target[,x.vars] %>% as.numeric()), 
                              rep(0, n_z), 
    ...
    for (year_t in year_list){
      ...
      obj_func <- c(rep(0, n_x),
                    rep(0, n_z),
                    period_sub_target[, c(y.vars)] %>% as.numeric())
      
      linear_constr_mat <- c(period_sub_target[, c(x.vars)] %>% as.numeric(), 
                             rep(0, n_z), 
                             rep(0, n_y))
      
      constr_mat <- rbind(linear_constr_mat, # 1
                          fixed_eff_constr_mat, # 1
      ...
      eff_s_t_list <- rbind.data.frame(eff_s_t_list, result_list)
